...
export function effectDecorator<S extends State = State>(handler: (effect: EffectHandler, rootState: Readonly<S>) => SagaIterator): EffectDecorator {
    return (target, propertyKey, descriptor) => {
        ...
        descriptor.value = function*(...args: any[]): SagaIterator {
            ...
            if (rootState) {
                yield* handler(originalEffect.bind(this, ...args), rootState);
            } else {
                throw new Error("decorator must be applied to ActionHandler effects");
            }
        };
